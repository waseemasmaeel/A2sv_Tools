...
ssl3_cipher = {
    ...
    b"\x00\x1c": "SSL_FORTEZZA_KEA_WITH_NULL_SHA",
    b"\x00\x1d": "SSL_FORTEZZA_KEA_WITH_FORTEZZA_CBC_SHA",
    b"\x00\x1e": "SSL_FORTEZZA_KEA_WITH_RC4_128_SHA",
    ...
    b"\x00\x00": "TLS_EMPTY_RENEGOTIATION_INFO_SCSV",
...
def m_poodle_run(strHost, iPort, displayMode):
    ...
    for strVer in ["SSLv3"]:
        ...
        while iCount < 5:
            ...
            if lstRecords is not None and len(lstRecords) > 0:
        ...
        if not (fServerHello and fCert):
            showDisplay(" - [LOG] Invalid SSLv3 handshake.")
        ...
        try:
            ...
            pass
    if iVulnCount > 0:
        showDisplay(" - [LOG] Allow SSLv3 Protocol")
        return "0x01"
        quit(1)
    else:
        return "0x00"
