...
def m_ccsinjection_run(strHost, iPort, displayMode):
    ...
    for strVer in ["TLSv1.2", "TLSv1.1", "TLSv1", "SSLv3"]:
        ...
        try:
            ...
            showDisplay("Failure connecting to %s:%d." % (strHost, iPort))
            quit()
        s.send(strHello)
        iCount = 0
        fServerHello = False
        ...
        if not (fServerHello and fCert):
            showDisplay(" - [LOG] %s Invalid handshake." % (strLogPre))
        elif len(recv) > 0:
            if ord(recv[0]) == 22:
                iCount = 0
                strChangeCipherSpec = b"\x14"
                strChangeCipherSpec += dSSL[strVer]
                strChangeCipherSpec += b"\x00\x01"  # Len
                strChangeCipherSpec += b"\x01"  # Payload CCS
                ...
                while iCount < 5:
                    ...
                    try:
                        ...
                        showDisplay("Connection closed unexpectedly.")
                        fVuln = False
                        break
                    if (len(recv) > 0):
                        strLastMessage = recv
                ...
                if fVuln:
                    try:
                        ...
                        fVuln = False
                if fVuln:
                    showDisplay(" - [LOG] %s %s:%d may allow early CCS" % (strVer, strHost, iPort))
                    iVulnCount += 1
                else:
                    showDisplay(" - [LOG] %s %s:%d rejected early CCS" % (strVer, strHost, iPort))
        else:
            showDisplay(" - [LOG] %s No response from %s:%d" % (strVer, strHost, iPort))
        try:
            s.close()
        except:
            pass
